...
function wpmtst_single_form_field( $field ) {
	...
	if ( 'checkbox' !== $field['input_type'] ) {
		if ( ! isset( $field['show_label'] ) || $field['show_label'] ) {
			$label_aria_hidden = ( 'rating' === $field['input_type'] ) ? ' aria-hidden="true"' : '';
			printf( '<label for="wpmtst_%s" class="%s"%s>%s</label>', esc_html( $field['name'] ), esc_attr( wpmtst_field_label_classes( $field['input_type'], $field['name'] ) ), $label_aria_hidden, wp_kses_post( wpmtst_form_field_meta_l10n( $field['label'], $field, 'label' ) ) );
