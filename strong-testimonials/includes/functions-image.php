...
function wpmtst_thumbnail_img_platform_general( $img, $post_id, $size ) {
	...
	return sprintf( '<img src="%s" %s %s/>', esc_url( $platform_user_photo ), $width ? "width='{$width}'" : '', $height ? "height='{$height}'" : '' );
}
...
function wpmtst_thumbnail_img_platform_woocommerce( $img, $post_id, $size ) {
	...
	return sprintf( '<img src="%s" %s %s/>', esc_url( get_avatar_url( $email ) ), $width ? "width='{$width}'" : '', $height ? "height='{$height}'" : '' );
}
